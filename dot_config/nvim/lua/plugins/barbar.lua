return {
  ...
  config = function()
    ...
    -- Pin/unpin buffer
    map("n", "<A-p>", "<Cmd>BufferPin<CR>", { desc = "Pin Buffer", table.unpack(opts) })
    -- Close buffer
    map("n", "<C-c>", "<Cmd>BufferClose<CR>", { desc = "Close Buffer", table.unpack(opts) })
    -- Restore buffer
    map("n", "<A-S-c>", "<Cmd>BufferRestore<CR>", { desc = "Restore Buffer", table.unpack(opts) })
    -- Magic buffer-picking mode
    map("n", "<C-p>", "<Cmd>BufferPick<CR>", { desc = "Magic Buffer Select Mode", table.unpack(opts) })
